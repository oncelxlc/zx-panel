...
// Migrate 创建基础用户表、会话表和必要索引，操作可重复执行。
...
// CountUsers 返回系统用户总数，用于判断是否需要创建初始管理员。
...
// FindUserByUsername 查询账号及密码摘要，所有值均通过参数化 SQL 传入。
...
// CreateSession 保存令牌摘要，数据库中不落明文令牌。
...
// FindUserBySession 校验会话存在且未过期，并返回关联的启用用户。
...
// DeleteSession 删除指定会话；不存在时仍视为注销成功。
